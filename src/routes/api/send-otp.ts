  ...
  POST: async () => {
    try {
      ...
      await transporter.sendMail({
        from: `"MP Repair Security" <${process.env.GMAIL_USER}>`,
        to: OWNER_EMAIL,
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 12px;">
            ...
            <div style="background: #f1f5f9; border-radius: 8px; padding: 20px; text-align: center; margin: 20px 0;">
              <div style="font-size: 14px; color: #64748b; margin-bottom: 8px;">Your one-time password</div>
              <div style="font-size: 40px; font-weight: bold; letter-spacing: 10px; color: #3b82f6;">${otp}</div>
            </div>
            <p style="color: #64748b; font-size: 13px;">⏱️ This OTP expires in <strong>5 minutes</strong>.</p>
